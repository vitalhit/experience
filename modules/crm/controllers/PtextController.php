...
/**
 * PromoController implements the CRUD actions for Ptext model.
 */
class PtextController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionIndex($plist = false)
    {
        $query = Ptext::find();
    ...
    {
        $model = Ptext::find()->where(['ptext.id'=>$id])->one();
        if (empty($model)) {
            Yii::$app->getSession()->setFlash('danger', 'нет такого ПТекста.');
    ...
    /**
     * Updates an existing Ptext model.
    ...
    /**
     * Deletes an existing Ptext model.
    ...
    /**
     * Finds the Ptext model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Ptext the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Ptext::findOne($id)) !== null) {
            return $model;
        }
