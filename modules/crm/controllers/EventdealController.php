...
class EventdealController extends Controller
{
    ...
    { 
        $query = EventDeal::find()->orderBy(['id' => SORT_ASC])->joinWith('event')->joinWith('event.biblioevents');
        $countQuery = clone $query;
        $pages = new Pagination(['totalCount' => $countQuery->count(), 'PageSize' => 500]);
        $eventdeals = $query->offset($pages->offset)
        ->limit($pages->limit) 
        ->all();

        //echo "<pre>"; print_r($eventdeals); echo "</pre>";
        
        $for_sum = EventDeal::find()->where(['deal_id' => $deal_id ])->all();
        $sum_deals = 0;
        ...
        return $this->render('index.twig', ['eventdeals' => $eventdeals,'pages' => $pages, 'deal' => $deal_id, 'sum_deals' => $sum_deals]);
    }
    ...
    {
        $model = EventDeal::find()->where(['event_deal.id'=>$id])->one();
        if (empty($model)) {
    ...
    public function actionCreate_old() // удалить (210222)
    {
        $model = new EventDeal();
    ...
    /**
     * Updates an existing EventDeal model.
    ...
    {
        if (($model = Eventdeal::findOne($id)) !== null) {
            return $model;
        }
