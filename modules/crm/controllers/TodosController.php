...
/**
 * PromoController implements the CRUD actions for Todos model.
 */
class TodosController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        return $this->render('index.twig', ['todos' => $todos,'pages' => $pages, 'steps' => $steps]);
    }
    ...
    {
        $model = Todos::find()->where(['todos.id'=>$id])->one();
        if (empty($model)) {
            Yii::$app->getSession()->setFlash('danger', 'нет такого розыграша.');
            return $this->redirect(['/crm/todos']);
        }
