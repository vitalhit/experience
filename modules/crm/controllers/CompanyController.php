...
class CompanyController extends Controller
{
    ...
    public function actionMy($ord = false)
    {
        ...
        if (!empty($ord)) {
            $companies = Companies::find()->where(['id' => $comp_ids])->orderBy($ord)->asArray()->all();
        } else {
            $companies = Companies::find()->where(['id' => $comp_ids])->asArray()->all();
            $ord = null;
        }
        return $this->render('my.twig', ['companies' => $companies, 'ord' => $ord]);
    }


    public function actionAbout($id = false, $gourl = false)
    {
        // Если нет персоны - редирект на создание
        if (empty(Persons::isPerson())) {
            Yii::$app->getSession()->setFlash('danger', 'Заполните профиль!');
        ...
        if (!empty($id)) {
            $company = Companies::setCompany($id);
        }
    ...
    public function actionDohodOLD()
    {
        ...
        if (empty($get['year'])) {
            if ($get['year'] ?? Null == "all") {
                $year = $contract->date;
                $formonth = $contract->date;
            } else {
                $year = date("Y") . "-01-01";
                $formonth = date("Y") . "-01-01";
            }
        } else {
            $year = $get['year'];
            $formonth = "2018-01-01";
        ...
        $next = date('Y-m-d', mktime(0, 0, 1, date('m', strtotime($formonth)), 01, date('Y', strtotime($year))));

        if (empty($next) or $next < "2018-01-01") {
            $next = "2018-01-01";
        } // year of start igoevent.com
    ...
    public function actionDohod()
    {
        ...
        return $this->render('dohod_new.twig', ['time' => microtime(true) - $t1, 'itog' => $itog, 'payments' => $payments, 'akt' =>  $akt]);
    }
    ...
    public function actionPayments()
    {
        ...
        $payments = CompanyPayments::find()->where(['company_id' => $company->id])->orderBy(['id' => SORT_DESC])->all();
        ...
        return $this->render('payments.twig', ['payments' => $payments]);
    }
    ...
    public function actionVuvod()
    {
        ...
        if ($payment->save()) {
            ...
            return $this->redirect(['/crm/company/payments']);
        };
    }
