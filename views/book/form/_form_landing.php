...
<div class="contragent-form">
    ...
    <?= $form->field($model, 'company_id')->hiddenInput(['value' => $booking->company_id ])->label(false); ?>
    <?= $form->field($model, 'list_id')->hiddenInput(['value' => $booking->list_id ])->label(false); ?>
    ...
    <?= $form->field($model, 'from_url')->hiddenInput(['value' => 'https://igoevent.com/'.$city->alias.'/book/'.$booking->alias])->label(false); ?>
    <div class="col-xs-12 col-lg-12">
    <?php 
        if ($event_id == '' && $booking->biblioevent_id == ''){
            }
            elseif( is_numeric($booking->biblioevent_id)){
                echo $form->field($model, 'event_id')->dropDownList(ArrayHelper::map($events, 'id', function ($data) { return Yii::$app->formatter->asDate($data->date); }), ['value' => $booking->biblioevent_id ??'', 'prompt' => 'Выберите ...'])->label('Выберите дату');
            }
            else{
                echo $form->field($model, 'event_id')->dropDownList(ArrayHelper::map($events, 'id', function ($data) { return Yii::$app->formatter->asDate($data->date); }), ['value' => $event_id??'', 'prompt' => 'Выберите ...'])->label('Выберите дату');
            }
            ?>
    </div>
